...
import tunanh.documentation.xs.simpletext.view.DocAttr;
import tunanh.documentation.xs.simpletext.view.IRoot;
import tunanh.documentation.xs.simpletext.view.IView;
import tunanh.documentation.xs.simpletext.view.PageAttr;
import tunanh.documentation.xs.simpletext.view.ParaAttr;
import tunanh.documentation.xs.simpletext.view.TableAttr;
...
{   
        ...
        TableView tableView, long startOffset, int x, int y, int w, int h, int flag, boolean isBreakPages)
    {
            ...
            || (breakRowElement != null && isBreakPages))
        {
            ...
            else if (breakType == WPViewConstant.BREAK_LIMIT && !breakPagesCell.isEmpty()
                || isRowBreakPages)
            {
                breakRowElement = (RowElement)rowView.getElement();
            }
        ...
        RowView rowView, long startOffset, int x, int y, int w, int h, int flag, boolean isBreakPages)
    {
            ...
            /*|| breakPagesCell.size() > 0 && isBreakPages*/)
        {
            ...
            isNullCell = false;
            // 表格跨页
            if (isBreakPages && !breakPagesCell.isEmpty())
            {
