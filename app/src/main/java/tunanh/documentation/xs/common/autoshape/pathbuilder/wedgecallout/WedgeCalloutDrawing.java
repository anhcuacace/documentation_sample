 ...
 * 时间:               上午9:25:20
 */
package tunanh.documentation.xs.common.autoshape.pathbuilder.wedgecallout;
    ...
    private static Path getWedgeRectCalloutPath(AutoShape shape, Rect rect)
    {
        ...
        else
        {
            ...
            if (values != null && values.length >= 2)
            {
                if (values[0] != null)
                {
                    x = rect.width() * values[0] - (float) rect.width() / 2;
                }
                if (values[1] != null)
                {
                    y = rect.height() * values[1] - (float) rect.height() / 2;
                }
            }
        }
    ...
    private static Path getCloudCalloutPath(AutoShape shape, Rect rect)
    {
        ...
        else
        {
            if(values != null && values.length >= 2)
            {
                if (values[0] != null)
                {
                    adj1 = Math.round(rect.width() * values[0] - (float) rect.width() / 2);
                }
                if (values[1] != null)
                {
                    adj2 = Math.round(rect.height() * values[1] - (float) rect.height() / 2);
                }
            } 
            else
            {
