...
import tunanh.documentation.xs.common.autoshape.AutoShapeDataKit;
...
import tunanh.documentation.xs.common.autoshape.pathbuilder.LineArrowPathBuilder;
...
import tunanh.documentation.xs.common.bg.PatternShader;
...
import tunanh.documentation.xs.common.shape.WPChartShape;
import tunanh.documentation.xs.common.shape.WPGroupShape;
import tunanh.documentation.xs.common.shape.WPPictureShape;
import tunanh.documentation.xs.common.shape.WatermarkShape;
import tunanh.documentation.xs.constant.MainConstant;
import tunanh.documentation.xs.constant.SchemeClrConstant;
...
import tunanh.documentation.xs.fc.dom4j.Document;
import tunanh.documentation.xs.fc.dom4j.Element;
import tunanh.documentation.xs.fc.dom4j.ElementHandler;
import tunanh.documentation.xs.fc.dom4j.ElementPath;
import tunanh.documentation.xs.fc.dom4j.io.SAXReader;
import tunanh.documentation.xs.fc.openxml4j.exceptions.InvalidFormatException;
...
import tunanh.documentation.xs.fc.ppt.reader.ThemeReader;
...
import tunanh.documentation.xs.simpletext.model.Style;
import tunanh.documentation.xs.simpletext.model.StyleManage;
...
public class DOCXReader extends AbstractReader
{
    ...
    private void openFile() throws Exception
    {
        ...
        if (zipPackage.getParts().isEmpty())
        {
    ...
    private void processSectionAttribute(Element sectPr)
    {
        ...
        if(docGrid != null)
        {
        	...
        	if("lines".equals(type) || "linesAndChars".equals(type) || "snapToChars".equals(type))
        	{
        		...
        		if(linePitch != null && !linePitch.isEmpty())
        		{        			
            ...
            if (shape.attribute("style") != null)
            {
                ...
                if (styleStr != null)
                {
                    ...
                    if(autoShape instanceof WPAutoShape && ((WPAutoShape)autoShape).getShapeType() == ShapeTypes.ArbitraryPolygon)
                    {
                    	...
                    	if(coordsize != null && !coordsize.isEmpty())
                    	{
                        ...
                        if (temps != null && temps[0] != null && temps[1] != null)
                        {
                            ...
                            else if ("rotation".equalsIgnoreCase(temps[0]))
                            {
                                if (temps[1].indexOf("fd") > 0)
                                {
                                    ...
                                    autoShape.setRotation((float) Integer.parseInt(temps[1]) / 60000);
                                }
                                else
                                {
...
//            }
    ...
    private int getRadialGradientPositionType(Element fill)
    {
		...
		if(focusposition != null && !focusposition.isEmpty())
		{
    ...
    private int getAutoShapeType(Element shape)
    {
        ...
        switch (name) {
            case "rect" -> shapeType = ShapeTypes.Rectangle;
            case "roundrect" -> shapeType = ShapeTypes.RoundRectangle;
            case "oval" -> shapeType = ShapeTypes.Ellipse;
            case "curve" -> shapeType = ShapeTypes.Curve;
            case "polyline" -> shapeType = ShapeTypes.DirectPolygon;
            case "line" -> shapeType = ShapeTypes.WP_Line;
        }
