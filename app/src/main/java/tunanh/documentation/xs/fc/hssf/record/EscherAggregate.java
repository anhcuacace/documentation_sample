...
import tunanh.documentation.xs.fc.ddf.DefaultEscherRecordFactory;
import tunanh.documentation.xs.fc.ddf.EscherBoolProperty;
import tunanh.documentation.xs.fc.ddf.EscherClientAnchorRecord;
...
import tunanh.documentation.xs.fc.ddf.EscherRecordFactory;
import tunanh.documentation.xs.fc.ddf.EscherSerializationListener;
import tunanh.documentation.xs.fc.ddf.EscherSpRecord;
import tunanh.documentation.xs.fc.ddf.EscherSpgrRecord;
import tunanh.documentation.xs.fc.ddf.EscherTextboxRecord;
import tunanh.documentation.xs.fc.hssf.model.AbstractShape;
import tunanh.documentation.xs.fc.hssf.model.CommentShape;
import tunanh.documentation.xs.fc.hssf.model.ConvertAnchor;
import tunanh.documentation.xs.fc.hssf.model.DrawingManager2;
import tunanh.documentation.xs.fc.hssf.model.TextboxShape;
...
import tunanh.documentation.xs.fc.hssf.usermodel.HSSFShapeContainer;
import tunanh.documentation.xs.fc.hssf.usermodel.HSSFShapeFactory;
...
public final class EscherAggregate extends AbstractEscherHolderRecord {
	...
	public static int shapeContainRecords(List records, int currentDrawingRecord)
	{
	        ...
	        && isObjectRecord( records, currentDrawingRecord + 1 ))
	    {
	        ...
	        count = 2;
	        //chart records
            if(record instanceof ObjRecord && ((ObjRecord) record).getSubRecords().get(0) instanceof CommonObjectDataSubRecord commonObjectDataSubRecord)
            {
...
//		}
		...
		int index = 0;
		if(!containerList.get(index).getChildContainers().isEmpty())
		{
	...
	private void convertRecordsToUserModel(EscherContainerRecord shapeContainer, Object model) 
	{
		...
		{
			EscherRecord r = it.next();
			if(r instanceof EscherSpgrRecord spgr)
			{
				// This may be overriden by a later EscherClientAnchorRecord

                if(model instanceof HSSFShapeGroup group)
				{
			...
			else if(r instanceof EscherTextboxRecord tbr)
			{
                Record obj = shapeToObj.get(tbr);
				if(obj instanceof TextObjectRecord textObj && model instanceof HSSFTextbox textbox)
				{
			...
			else if(r instanceof EscherClientDataRecord cdr && model instanceof HSSFChart)
            {
...
//		lastShapeId = 1024;
	...
	private void convertUserModelToRecords()
	{
		if ( patriarch != null )
		{
			...
			clearEscherRecords();
			if (!patriarch.getChildren().isEmpty())
			{
		...
		for ( Iterator iterator = shapes.iterator(); iterator.hasNext(); )
		{
			...
			else
			{
				...
				if ( shapeModel instanceof TextboxShape )
				{
					...
					if (shapeModel instanceof CommentShape comment){
