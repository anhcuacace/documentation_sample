...
import tunanh.documentation.xs.constant.wp.AttrIDConstant;
...
import tunanh.documentation.xs.fc.openxml4j.opc.ContentTypes;
...
import tunanh.documentation.xs.fc.openxml4j.opc.PackageRelationshipCollection;
...
import tunanh.documentation.xs.fc.ppt.reader.HyperlinkReader;
import tunanh.documentation.xs.fc.ppt.reader.LayoutReader;
import tunanh.documentation.xs.fc.ppt.reader.MasterReader;
import tunanh.documentation.xs.fc.ppt.reader.PictureReader;
import tunanh.documentation.xs.fc.ppt.reader.ReaderKit;
import tunanh.documentation.xs.fc.ppt.reader.SmartArtReader;
import tunanh.documentation.xs.fc.ppt.reader.StyleReader;
import tunanh.documentation.xs.fc.ppt.reader.TableStyleReader;
...
import tunanh.documentation.xs.system.StopReaderError;
        ...
        {       
            ...
            try
            {
                switch (name) {
                    ...
                    case "defaultTextStyle" -> processDefaultTextStyle(elem);
                    case "sldSz" -> setPageSize(elem);
                    case "sldId" -> addSlideID(elem);
                }
            }
            catch(Exception e)
            {
            ...
            try
            {
                ...
                else if ("sld".equals(elem.getName()))
                {
                    if (elem.attribute("showMasterSp") != null)
                    {
                        ...
                        if (val != null && !val.isEmpty() && Integer.valueOf(val) == 0)
                        {
                            showMasterSp = false;
                        }    
                    }
                ...
                {
                    //slide transition
                    pgSlide.setTransition(!elem.elements().isEmpty());
                }
                else
                {
...
//        processGroupShapeID(grpShape);
    ...
    private void processGroupShapeID(Map<Integer, List<Integer>> grpShape)
    {
        ...
        while(grpIDIter.hasNext())
        {
            ...
            for(int i = 0; i < grpCnt; i++)
            {
                ...
                if(childchildShape != null && !childchildShape.isEmpty())
                {
                    if(ids == null)
                    {
    ...
    private void processAnimation(PGSlide pgSlide, Element elem)
    {
        ...
        switch (sType) {
            case "entr" -> nType = ShapeAnimation.SA_ENTR;
            case "emph" -> nType = ShapeAnimation.SA_EMPH;
            case "exit" -> nType = ShapeAnimation.SA_EXIT;
            default -> {
                //path, verb, mediacall
                return;
            }
