...
import tunanh.documentation.xs.fc.hwpf.sprm.TableSprmUncompressor;
...
public final class TableRow extends Range
{
    ...
    private void initCells()
    {
        ...
        for (int p = 0; p < numParagraphs(); p++)
        {
            ...
            if (((!s.isEmpty() && s.charAt(s.length() - 1) == TABLE_CELL_MARK) || paragraph
                .isEmbeddedCellMark()) && paragraph.getTableLevel() == _levelNum)
            {
                ...
                if (cells.isEmpty() || cells.size() + 2 >= _tprops.getRgdxaCenter().length)
                {
