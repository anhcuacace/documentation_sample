...
public abstract class AbstractChart/* implements Serializable*/ 
{
  ...
  public Rectangle getTextSize(String text, float fontSize, float maxWidth, float maxHeight, Paint paint)
  {
	  ...
	  else
	  {
		  ...
		  while(!title.isEmpty() && titleHeight + lineHeight <= maxHeight )
		  {				  
			  ...
			  if(!title.isEmpty() && titleHeight + lineHeight * 2 > maxHeight)
			  {
  ...
  protected void drawTitle(Canvas canvas, String title, float zoom, float x, float y, float maxWidth, float maxHeight, Paint paint, float angle) 
  {
	  ...
	  
	  {
		  ...
		  else
		  {
			  ...
			  float[] charWidth = new float[]{};
			  float sum = 0;
			  while(!title.isEmpty() && sum + lineHeight <= maxHeight )
			  {				  
				  int cnt = paint.breakText(title, true, maxWidth, charWidth);
				  if(cnt == 0)
	  			  {
				  ...
				  title = title.substring(cnt);
				  if(!title.isEmpty() && sum + lineHeight * 2 > maxHeight)
				  {
   ...
   */
  protected int drawLegend(Canvas canvas, DefaultRenderer renderer, String[] titles, int left, int top, int width, Paint paint)
  {
	  if (renderer.isShowLegend()) 
	  {
			        ...
			        paint.setColor(categoryAxisTextColor);    
			  		...
			  		if (extraSize > width) 
					 {
			  			...
			  			while(!text.isEmpty())
			  			{
				  			...
				  			currentY += (float) Math.ceil(fm.descent - fm.ascent);
			  			}			  			
					 }
					 else
					 {
