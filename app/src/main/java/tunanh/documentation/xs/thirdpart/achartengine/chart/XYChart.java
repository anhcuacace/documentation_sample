 ...
 */
package tunanh.documentation.xs.thirdpart.achartengine.chart;
...
import tunanh.documentation.xs.common.BackgroundDrawer;
...
import tunanh.documentation.xs.thirdpart.achartengine.renderers.BasicStroke;
...
import tunanh.documentation.xs.thirdpart.achartengine.renderers.XYMultipleSeriesRenderer.Orientation;
...
public abstract class XYChart extends AbstractChart {
   ...
   * 
   *(non-Javadoc)
   * @see tunanh.documentation.xs.thirdpart.achartengine.chart.AbstractChart#setZoomRate(float)
   *
   */
  @Override
  ...
  {	  
  	  ...
  	  if(legendSize != null && (legendPos == LegendPosition_Top || legendPos == LegendPosition_Bottom))
	  {
  	  ...
  	  bottom -= (int) (fm.descent - fm.ascent);
  	  ...
  	  mTranslate = (float) Math.abs(width - height) / 2;
  	  if (mScale < 1)
  	  {
  		  mTranslate *= -1;
  	  }
  	  mCenter = new PointF((float) (x + width) / 2, (float) (y + height) / 2);
  	  if (rotate) 
  	  {
...
//    canvas.drawRect(mScreenR, paint);
    ...
    if (showLabels || showGrid) 
    {
      ...
      { 
    	  ...
    	  {
    		  xLabelsLeft += (int) (mRenderer.getLabelsTextSize() / 4);
    	  }
      }
      ...
      if (showLabels) 
      {
        ...
        if (or == Orientation.HORIZONTAL) 
        {
        	//draw chart title
            	...
            	drawTitle(canvas, mRenderer.getChartTitle(), 1.0f, x + (float) width / 2, y + mRenderer.getChartTitleTextSize() * mRenderer.getZoomRate() * 2,
       	 	...
       	 	{        			 
       	 		...
       	 		{
       	 			yTitleTop = y + titleAreaSize.height + (float) height / 2;
       	 		}
       	 		else
       	 		{
       	 			yTitleTop = y + (float) height / 2;
       	 		}
       	 	...
       	 	if(xTitleAreaSize != null)
       	 	{
       	 		...
       	 		if(yTitleAreaSize != null)
       	 		{
       	 			xTitleLeft = x + (float) (width + yTitleAreaSize.width) / 2;
       	 		}
       	 		else
       	 		{
       	 			xTitleLeft = x + (float) width / 2;
       	 		}
        ...
        {
        	drawText(canvas, mRenderer.getXTitle(), x + (float) width / 2, y + height, paint, -90);
        	drawText(canvas, mRenderer.getYTitle(), right + 20 * mRenderer.getZoomRate(), y + (float) height / 2, paint, 0);
        	paint.setTextSize(mRenderer.getChartTitleTextSize()* mRenderer.getZoomRate());
        	drawText(canvas, mRenderer.getChartTitle(), x, top + (float) height / 2, paint, 0);
        }
    ...
    if (or == Orientation.HORIZONTAL) 
    {
    	if(mRenderer.isShowLegend())
    	{
        	...
        	drawLegend(canvas, mRenderer, titles, legendLeft, legendTop, legendWidth, paint);
    	}
    ...
    else if (or == Orientation.VERTICAL) 
    {
    	...
    	drawLegend(canvas, mRenderer, titles, x + SSConstant.SHEET_SPACETOBORDER, y, width, paint);
    	transform(canvas, angle, false);
    }
  ...
  {
	  if(!mRenderer.getXTitle().isEmpty())
	  {
  ...
  {	  
	  if(!mRenderer.getYTitle().isEmpty())
	  {
		  //rotate 90
